    ...
    async def wrapped(update, context, *args, **kwargs):
        ...
        if user_id != int(os.getenv("AUTHORIZED_USER")):
            ...
            await update.message.reply_animation(
                "https://t.me/alexbossnfs",
                caption="Бот настроен не для вас! Напишите @alexbossnfs для получения доступа",
            )
            return
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "Начать новый диалог", callback_data="New_Conversation"
            ),
            InlineKeyboardButton(
                "Описание картинки", callback_data="Image_Description"
            ),
        ],
        [InlineKeyboardButton("История общений", callback_data="PAGE#1")],
    ]
...
async def start_over(update: Update, context: ContextTypes.DEFAULT_TYPE, conn) -> int:
    ...
    try:
        ...
        if gemini_chat or conversation_id:
            if "_SAVE" in query.data:
                ...
                logger.info(f"общение {conversation_id} сохранено в базу данных и закрыто")

            else:
                logger.info(f"общение {conversation_id} закрыто без сохранения")

            gemini_chat.close()
        else:
            logger.info("Нету чата для закрытия")
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "Начать новый чат", callback_data="New_Conversation"
            )
        ],
        [InlineKeyboardButton("Описание картинки", callback_data="Image_Description")],
        [InlineKeyboardButton("История общений", callback_data="PAGE#1")],
        [InlineKeyboardButton("Начать сначала", callback_data="Start_Again")],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    msg = await context.bot.send_message(
        query.message.chat.id,
        text="Привет. Это помощник Яндекс лицея. Спрашивайте свои вопросы",
        reply_markup=reply_markup,
    )
...
async def start_conversation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    message_content = "Вы запросили создать новый чат. Успешно!"

    conv_id = context.user_data.get("conversation_id")
    if conv_id:
        message_content = "Вы запросили продолжить чат. Успешно!"

    keyboard = [[InlineKeyboardButton("Вернуться в меню", callback_data="Start_Again")]]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def reply_and_new_message(
    ...
    keyboard = [[InlineKeyboardButton("Назад в меню", callback_data="Start_Again")]]
    reply_markup = InlineKeyboardMarkup(keyboard)

    msg = await update.message.reply_text(
        text="Ждите, идёт обработка запроса...",
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "Сохранить и выйти в меню", callback_data="Start_Again_SAVE"
            )
        ],
        [
            InlineKeyboardButton(
                "Вернуться в меню без сохранения", callback_data="Start_Again"
            )
        ],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def get_conversation_handler(
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "Продолжить чаты", callback_data="New_Conversation"
            )
        ],
        [
            InlineKeyboardButton(
                "Удалить чат(ы)", callback_data="Delete_Conversation"
            )
        ],
        [InlineKeyboardButton("Назад в меню", callback_data="Start_Again")],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def delete_conversation_handler(
    ...
    msg = await query.edit_message_text(
        text="История переписок успешно очищена. Вернуться в меню Начать новую переписку",
        reply_markup=reply_markup,
        parse_mode=ParseMode.MARKDOWN,
    )
    context.user_data["to_delete_message"] = msg
...
async def start_image_conversation(
    ...
    msg = await query.edit_message_text(
        f"Получиен запрос на описание картинки. Отправьте картинку",
        reply_markup=reply_markup,
    )
    context.user_data["to_delete_message"] = msg
...
async def generate_text_from_image(
    ...
    try:
        ...
        response = "Не получилось сгенерировать ответ. Попробуйте ещё раз"
...
async def get_conversation_history(
    ...
    if conversations:
        ...
        page_content = "Нету истории для очистки"
...
async def done(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("Описание картинки", callback_data="Image_Description")],
        [InlineKeyboardButton("Заново", callback_data="Start_Again")],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)

    await context.bot.send_message("Увидимся ещё!", reply_markup=reply_markup)
